...
#define	FLASH_DRAIN_TIME	 0.2 //100 units/3 minutes
...
bool CBasePlayer::ClientCommand(const CCommand& args)
{
	...
	if (stricmp(cmd, "test_SmokeGrenade") == 0)
	{
		...
		else if (stricmp(cmd, "about") == 0) // start gsmod shit
		{
			...
			Msg("GreenSoup's Mod v1.4\n");
			Msg("Build date: 21/01/2024\n");
...
#if 0 // BYE BYE!!
	// Update Flashlight
	/* ((m_flFlashLightTime) && (m_flFlashLightTime <= gpGlobals->curtime))
	{
		if (FlashlightIsOn())
		{
			if (m_iFlashBattery)
...
void CMovementSpeedMod::InputSpeedMod(inputdata_t& data)
{
	...
	if (pPlayer)
	{
		...
		/*ifHasSpawnFlags(SF_SPEED_MOD_SUPPRESS_WEAPONS) && pPlayer->GetActiveWeapon() == NULL)
			{
